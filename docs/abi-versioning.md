...
The ABI is the `lca:ext` WIT package: the worlds `provider`, `tool`, `command`, `hooks`, and `ui`, and the host import interfaces they reference. It also covers the manifest schema, because a manifest the host cannot parse is a load failure like any other.
...
| Add a new function to an existing world | Breaking for that world |
...
Adding a function to a world is breaking because a component that does not export it fails to satisfy the world. Where a new function is optional in spirit, the correct shape is a separate world that an extension opts into by declaring it in the manifest.
...
An extension targeting an older version than the window allows is refused at load time. The host disables it for the session, reports whether a compatible version exists in the registry, names the command that fixes it, and continues. A stale extension does not stop the agent from starting.
