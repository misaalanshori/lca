...
**Quarantine ledger (age clock).** The Windows-only ignores above are the project's quarantine set, opened together on 2026-09-25 so the testing plan's stale-quarantine rule has a date to measure from. The ConPTY real-terminal test joins this ledger if it proves flaky on hosted runners. Under section 13 of the testing plan, a quarantined test past the set age without a fix blocks new quarantines from the same crate until it is resolved: an ignore here is a dated debt with an owner, not a resting place.
