...
**Credentials.** The credential store's owner-only mode (`0o600`, NFR-14) is set on Unix. On Windows the file is created under the user profile's data directory and relies on that directory's default ACL to restrict other users; no explicit security descriptor is written. This is the documented 1.0 position (ADR-0021); a Windows ACL would need a Windows API call in the credential path.
